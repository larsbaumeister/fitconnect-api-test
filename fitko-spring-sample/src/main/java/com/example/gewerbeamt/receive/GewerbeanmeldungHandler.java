 ...
 * submission, then publishes it as an {@link AntragReceivedEvent};
 * {@link AntragEventListener} is a meta-{@code @EventListener} that also
 * filters by LeiKa key, so this method only sees Gewerbeanmeldungen. Other
 * services would go to other listeners (see {@link AntragAuditListener} for
 * an unfiltered one).
 *
 * <p><b>Idempotency is mandatory.</b> Delivery is at-least-once: if no
 * listener calls {@link ReceivedAntrag#accept()} / {@link ReceivedAntrag#reject},
 ...
 * {@link ReceivedAntragStore#save} dedupes on submission id for that reason.
 *
...
public class GewerbeanmeldungHandler {
    ...
    public GewerbeanmeldungHandler(ReceivedAntragStore store) {
        this.store = store;
    }
    ...
    public void onGewerbeanmeldung(AntragReceivedEvent event) {
        ...
        String xml = antrag.getDataAsString();
        if (!looksLikeGewerbeanmeldung(xml)) {
            ...
            log.warn("Rejecting submission {}: payload is not a Gewerbeanmeldung document", antrag.getSubmissionId());
            antrag.reject(new DataSchemaViolation());
            return;
        }
        ...
        if (firstTime) {
            log.info("Stored Gewerbeanmeldung from submission {} (case {}), {} attachment(s)",
                    received.submissionId(), received.caseId(), antrag.getAttachments().size());
        } else {
